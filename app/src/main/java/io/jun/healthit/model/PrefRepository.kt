...
class PrefRepository {
        ...
        fun getAlertSettings(context: Context): String {
            val preferences = PreferenceManager.getDefaultSharedPreferences(context)
            return preferences.getString("alert", if(Setting.IN_KOREA)"vibrate_and_ring" else "Vibrate and Ring")!!
        }
        ...
        fun getRingSettings(context: Context): String {
            val preferences = PreferenceManager.getDefaultSharedPreferences(context)
            return preferences.getString("ring", if(Setting.IN_KOREA)"light_weight_babe" else "Light Weight Babe")!!
        }
        ...
        fun getTagSettings(context: Context, forSort: Boolean): ArrayList<Tag> {
            ...
            if(forSort) tagList.add(Tag(R.drawable.transparent, if(Setting.IN_KOREA)"전체보기" else "All"))
        ...
        fun getTemplate(key: Int, context: Context): List<Record> {
            ...
            return Gson().fromJson(preferences.getString(
                template + key,
                defJsonValue
            ), type)
        }
        ...
        fun getTemplateName(key: Int, context: Context): String? {
            val preferences = PreferenceManager.getDefaultSharedPreferences(context)
            return preferences.getString(templateName + key, if(Setting.IN_KOREA)"루틴$key (설정되어 있지 않음)" else "Routine$key (Not set yet)")
        }
